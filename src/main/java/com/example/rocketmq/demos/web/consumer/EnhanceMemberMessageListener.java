...
import com.example.rocketmq.demos.web.util.EnhanceMessageHandler;
...
public class EnhanceMemberMessageListener extends EnhanceMessageHandler<MemberMessage> implements RocketMQListener<MemberMessage> {
    ...
    protected void handleMessage(MemberMessage message) throws Exception {
        ...
        System.out.println("业务消息处理:" + message.getContent());
        if ("OA".equals(message.getSource())){
            log.error("不能处理来源OA的消息");
            throw new RuntimeException("不能处理来源OA的消息");
        }
    }
